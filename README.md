...
Wireshark was used to capture traffic between **Open vSwitch** and **pfSense** to validate correct VLAN behavior and confirm the difference between **Layer 2 switching** (same VLAN) and **Layer 3 routing** (inter-VLAN).

---

## Same-VLAN Traffic (Layer 2 Switching)
When two hosts are in the **same VLAN**, the switches can forward frames at **Layer 2** without sending traffic to pfSense for routing. In this scenario, you should see **minimal traffic involving pfSense**, because no routing is required.

![Same VLAN Capture](images/part2-same-vlan-capture.png)

---

## Same-VLAN Traffic on VLAN 2 (Layer 2 Switching)
This capture confirms VLAN 2 hosts can communicate using **Layer 2 forwarding**. Because both devices are in the same VLAN, traffic is handled by the switch (not routed by pfSense).

![VLAN 2 Same VLAN Traffic](images/part2-vlan2-same-vlan.png)

---

## Inter-VLAN Traffic: vWorkstation (VLAN 1) → PC2 (VLAN 2)
When traffic crosses VLAN boundaries, it must be routed by pfSense (router-on-a-stick). In this scenario, you should see traffic flowing **to pfSense and back**, because Layer 3 routing is required between VLAN 1 and VLAN 2.

![Inter-VLAN Workstation to PC2](images/part2-inter-vlan-ws-to-pc2.png)

---

## Inter-VLAN Traffic: PC2 (VLAN 2) → PC3 (VLAN 1)
This capture validates inter-VLAN routing in the opposite direction (VLAN 2 → VLAN 1). Successful ICMP traffic here confirms:
- VLAN tagging and trunking are correct
- pfSense interfaces/gateways are configured correctly
...
- pfSense inter-VLAN routing (router-on-a-stick)  
